...
<body>
    <!---------- header ---------->
      ...
      <!-- login -->
        ...
        ?>
        <h1>자유게시판</h1>
        <form action="./free_updateprocess.php" method="post">
        <!-- <input type="text" name="users_id" value="<?=$users_id?>"/> -->
        <input type="text" name="id" value="<?=$id?>"/>
        <input type="text" name="title" value="<?=$title?>" required><br>
        <textarea name="contents" rows="10" cols="50" required><?=$contents?></textarea><br>
        <input type="submit" value="수정">
        <button type="button" value="cancle" onclick="location.href='./free_detailview.php?id=<?=$id?>'">취소</button>
        </form>
